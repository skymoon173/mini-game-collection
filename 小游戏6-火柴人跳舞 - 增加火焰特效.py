...
class StickFigure:
    ...
    def draw(self, move=None):
        ...
        if move == 'move_1':
            ...
            self.draw_fire_effect(self.x - 20, self.y)  # 左手火焰特效
        elif move == 'move_2':
            # 双臂上举
            pygame.draw.line(screen, BLACK, (self.x, self.y + 20), (self.x - 20, self.y), 2)
            pygame.draw.line(screen, BLACK, (self.x, self.y + 20), (self.x + 20, self.y), 2)
            self.draw_fire_effect(self.x - 20, self.y)  # 左手火焰特效
            self.draw_fire_effect(self.x + 20, self.y)  # 右手火焰特效
        elif move == 'move_3':
            # 左臂下放，右臂上举
            pygame.draw.line(screen, BLACK, (self.x, self.y + 20), (self.x - 20, self.y + 40), 2)
            pygame.draw.line(screen, BLACK, (self.x, self.y + 20), (self.x + 20, self.y), 2)
            ...
            self.draw_fire_effect(self.x + 20, self.y + 10)  # 右手火焰特效
        elif move == 'move_6':
            # 左臂向前，右臂下放
            pygame.draw.line(screen, BLACK, (self.x, self.y + 20), (self.x - 20, self.y + 10), 2)
            pygame.draw.line(screen, BLACK, (self.x, self.y + 20), (self.x + 10, self.y + 40), 2)
            self.draw_fire_effect(self.x - 20, self.y + 10)  # 左手火焰特效
        else:
            # 默认双臂放下
            pygame.draw.line(screen, BLACK, (self.x, self.y + 20), (self.x - 10, self.y + 40), 2)
            pygame.draw.line(screen, BLACK, (self.x, self.y + 20), (self.x + 10, self.y + 40), 2)

    def draw_fire_effect(self, x, y):
        # 动态变化火焰的形状和颜色
        fire_height = random.randint(10, 20)
        fire_points = [
            (x - random.randint(3, 5), y),
            (x, y - fire_height),
            (x + random.randint(3, 5), y)
        ]
        pygame.draw.polygon(screen, RED, fire_points)
        pygame.draw.polygon(screen, ORANGE, [
            (x - random.randint(2, 4), y),
            (x, y - fire_height + random.randint(2, 5)),
            (x + random.randint(2, 4), y)
        ])
        pygame.draw.polygon(screen, YELLOW, [
            (x - random.randint(1, 2), y),
            (x, y - fire_height + random.randint(4, 8)),
            (x + random.randint(1, 2), y)
        ])
